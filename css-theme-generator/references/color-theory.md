...
> 🔓 **创新与越界特权 (Innovation Privilege)**：
> 本文件提供的是"安全兜底"推导规则。如果你判断用户的需求是极具个性的特殊风格（如赛博朋克、复古 8bit、玻璃拟物风、废土风等），你**可以且应当突破**以下限制（如背景亮度限制、对比度限制），使用更激进、更大胆的颜色组合来达到最佳的视觉冲击力。你可以将这些高级想法写在模板末尾的 `{{ADVANCED_EFFECTS_CSS}}` 中。
